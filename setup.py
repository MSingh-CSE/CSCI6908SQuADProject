...
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename, "r") as fh:
        ...
        for article in tqdm(source["data"]):
            if data_type == "train":
                start = 0
                end = len(article["paragraphs"])
            elif data_type == "dev":
                start = 0
                end = len(article["paragraphs"])//2
            elif data_type == "test":
                start = len(article["paragraphs"])//2
                end = len(article["paragraphs"])
            for para in article["paragraphs"][start:end]:
...
if __name__ == '__main__':
    ...
    if args_.include_test_examples:
        args_.test_file = url_to_data_path(args_.dev_url)
